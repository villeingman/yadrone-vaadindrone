...
import de.yadrone.apps.controlcenter.DataSender;
...
public class KeyboardLayoutPanel extends JPanel implements ICCPlugin {
	...
	private DataSender myDataSender;

	public KeyboardLayoutPanel() {
		myDataSender = new DataSender();

		loadImage(CCPropertyManager.getInstance()
				.isKeyboardCommandManagerAlternative());
		...
		originalButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				CCPropertyManager.getInstance()
						.setKeyboardCommandManagerAlternative(false);
				loadImage(CCPropertyManager.getInstance()
						.isKeyboardCommandManagerAlternative());
				repaint();
			}
		});
		originalButton.setSelected(!CCPropertyManager.getInstance()
				.isKeyboardCommandManagerAlternative());
		...
		alternativeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				CCPropertyManager.getInstance()
						.setKeyboardCommandManagerAlternative(true);
				loadImage(CCPropertyManager.getInstance()
						.isKeyboardCommandManagerAlternative());
				repaint();
			}
		});
		alternativeButton.setSelected(CCPropertyManager.getInstance()
				.isKeyboardCommandManagerAlternative());
		...
		add(originalButton, new GridBagConstraints(0, 0, 1, 1, 0, 1,
				GridBagConstraints.FIRST_LINE_START, GridBagConstraints.NONE,
				new Insets(0, 0, 0, 0), 0, 0));
		add(alternativeButton, new GridBagConstraints(1, 0, 1, 1, 1, 1,
				GridBagConstraints.FIRST_LINE_START, GridBagConstraints.NONE,
				new Insets(0, 0, 0, 0), 0, 0));
	}
	...
	private void loadImage(boolean isAlternative) {
		ImageIcon icon = new ImageIcon(
				KeyboardLayoutPanel.class.getResource("keyboard_control"
						+ (isAlternative ? "_alternative" : "") + ".png"));
	...
	private Image getScaledImage() {
		return originalImage.getScaledInstance(getWidth(), getHeight(),
	...
	public void activate(IARDrone drone) {
		...
		myDataSender.begin(drone);

		if (CCPropertyManager.getInstance()
				.isKeyboardCommandManagerAlternative())
			cmdManager = new KeyboardCommandManagerAlternative(drone);
		else
			cmdManager = new KeyboardCommandManager(drone);
		...
		// drone
		System.out
				.println("KeyboardLayoutPanel: grab the whole keyboard input from now on ...");
	...
	public void deactivate() {
		...
		System.out
				.println("KeyboardLayoutPanel: release key event dispatcher ...");
